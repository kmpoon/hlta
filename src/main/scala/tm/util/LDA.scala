...
object ConvertDataToLDAFormat {
  class Conf(args: Seq[String]) extends Arguments(args) {
    ...
    val data = Reader.readARFF(inputFile)
    if(conf.binaryValue())
      data.binary().saveAsLda(name+".lda.txt", name+".vocab.txt")
    else
      data.saveAsLda(name+".lda.txt", name+".vocab.txt")
  }
