...
public class PEM {
	...
	// private LTM _model;
	protected BayesNet _modelEst;

	/**
	 * Original data.
	 */
	protected static DataSet _Origdata;
	 ...
	 */
	protected double _UDthreshold;

	/**
	 * Threshold for EM.
	 */
	protected double _emThreshold;

	/**
	 * Parameter for EM.
	 */
	protected int _EmMaxSteps;

	/**
	 * Parameter for EM.
	 */
	protected int _EmNumRestarts;
	/**
	 * The maximum number of latent variables at top level
	 */
	protected int _maxTop;
	/**
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 10) {
			System.err.println("Usage: java FastHLTA model originaldata testdata EmMaxSteps EmNumRestarts EM-threshold UDtest-threshold modelName  MaxIsland MaxTop");
			System.exit(1);
		}
	...
	public LTM FastHLTA_learn() throws FileNotFoundException,
		...
		//	Alayer.saveAsBif("Example level"+(level-1));
			...
			// CurrentModel.saveAsBif("Hierarchical-LTM-Level-" + (level - 1) +
			// ".bif");
	...
	//	CurrentModel.saveAsBif(_modelname+"_beforeEM.bif");
		...
		/*StochasticEm emLearner = new StochasticEm();
		emLearner.setDataFileName(_originalDataFileName);
		emLearner.setLocalMaximaEscapeMethod("ChickeringHeckerman");
		emLearner.setMaxNumberOfSteps(50);
		emLearner.setNumberOfRestarts(1);
		emLearner.setReuseFlag(true);
	...
	private LTM BuildLatentTree(DataSet _data) throws FileNotFoundException, UnsupportedEncodingException {
		...
		latentTree.saveAsBif("latentTree");
	 ...
	 */
	protected Map<Variable, Map<Variable, Double>> computeMisPar(
			ArrayList<Variable> bestPair, DataSet _data) {
	...
	protected Map<Variable, Map<Variable, Double>> computeMisByCount(
		...
		EmpiricalMiComputerForBinaryData computer =
				new EmpiricalMiComputerForBinaryData(_data, vars);
		double[][] miArray = computer.computerPairwise();

		return processMi(bestPair, miArray, vars);
	}

	private static Map<Variable, Map<Variable, Double>> processMi(
	 ...
	 * of variables with the largest MI to the head of the list.
	 */
	private Map<Variable, Map<Variable, Double>> computeMisSeq(
			ArrayList<Variable> bestPair, DataSet _data) {
		// initialize the data structure for pairwise MI
		Map<Variable, Map<Variable, Double>> mis =
...
private LTM BuildHierarchy(LTM OldModel, LTM tree) {
	...
	private void AssignmentOnSingleLTM(DataSet _data) {

		ArrayList<DataCase> data = _data.getData();

		Variable[] varName = new Variable[_hierarchies.keySet().size()];
		int index = 0;
		for (Variable latent : _hierarchies.keySet()) {
			/*
			 * Variable clone =
			 * _hierarchies.get(latent).getRoot().getVariable(); varName[index]
			 * = new Variable(clone.getName(), clone.getStates());
			 */
			varName[index] = _hierarchies.get(latent).getRoot().getVariable();
			index++;
		}

		_newdata = new int[data.size()][varName.length];
		_workingData = new DataSet(varName);
		if (_latentPosts.isEmpty()) {
			for (int i = 0; i < varName.length; i++) {
				LTM subModel = _hierarchies.get(varName[i]);

				updateStatsAndAssign(subModel, _data, i);
			}
		}

		for (int j = 0; j < data.size(); j++) {
			_workingData.addDataCase(_newdata[j], data.get(j).getWeight());
		}
	 ...
	 double Loglikelihood=
	  ScoreCalculator.computeLoglikelihood((BayesNet)_modelEst2, _Origdata); double
	  perLL = Loglikelihood/_Origdata.getTotalWeight();
	  System.out.println("PerP(D|theta)_ "+_modelEst2.getName()+ "=  "+perLL);
	 }
	 
	// private void evaluateTrue(){
	// double Loglikelihood=
	// ScoreCalculator.computeLoglikelihood((BayesNet)_model, _test);
	 ...
	 */
	private void updateStatsAndAssign(LTM subModel, DataSet _data, int VarNum) {
		BeliefNode root = subModel.getRoot();
	 ...
	 * @return
	 */
	private BayesNet BuildChowLiu(BayesNet model) {

		Set<Variable> TopVars = model.getInternalVars("BayesNet");
		UndirectedGraph mst = learnMaximumSpanningTree(TopVars, _workingData);
		...
		while (!frontier.isEmpty()) {
			AbstractNode node = frontier.poll();
			DirectedNode dNode = (DirectedNode) model.getNode(node.getName());

			for (AbstractNode neighbor : node.getNeighbors()) {
				DirectedNode dNeighbor =
						(DirectedNode) model.getNode(neighbor.getName());
				if (!dNode.hasParent(dNeighbor)) {
					model.addEdge(dNeighbor, dNode);
					frontier.offer(neighbor);
				}
			}
		}
	...
	private UndirectedGraph learnMaximumSpanningTree(Set<Variable> TopVars,
			DataSet _data) {
		// initialize the data structure for pairwise MI
		List<StringPair> pairs = new ArrayList<StringPair>();

		// the collection of latent variables.
		List<Variable> vars = new ArrayList<Variable>(TopVars);
